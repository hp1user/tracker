# Windows Time Tracker (Local-Only) - Project Manual

This repository contains a production-ready, local-only time-tracking application for Windows. It runs quietly in the system tray, polls active windows at a user-customizable interval, ignores minimized applications, aggregates screen time locally in a SQLite database, and displays metrics on a modern dark-mode CustomTkinter dashboard with structured category grids and pop-up calendars.

---

## Directory Structure
Create these 6 files inside your project folder:
```
tracker/
├── requirements.txt   # Pip package dependencies
├── database.py       # SQLite schema, settings, categories, and calendar queries
├── tracker.py        # Background win32 window and idle tracker (dynamic interval & exclusions)
├── tray.py           # System tray icon & right-click menu management
├── ui.py             # CustomTkinter dashboard & pop-up calendar modals
└── main.py           # Entry point and coordinator (manages config.json)
```
...
Run the app quietly in the background:
```powershell
python main.py
```
*(To run it without showing a terminal window in the background during script execution, you can run `pythonw main.py` instead).*

### 3. Open the Dashboard
* The application **starts silently in your Windows System Tray** (bottom-right corner, next to the clock. Click the `^` arrow if hidden).
* Right-click the blue clock icon and select **Open Dashboard**.
* Clicking the **"X"** close button on the dashboard will hide the window back to the tray rather than exiting the application. To exit fully, right-click the tray icon and select **Quit**.

---

## How to Compile into a Standalone `.exe`

To compile the entire project (GUI, SQLite, background threads, and system tray icon) into a single, standalone, windowless `.exe` file:
...
2. Navigate to your project directory and run:
   ...
   ```
3. Locate the compiled executable in:
...
1. **Buttery-Smooth Resizing (Lag-Free)**:
   - Outer layout panels and repeating row widgets (`AppRow` and `CategorySettingsRow`) inherit from standard `tkinter.Frame` rather than `ctk.CTkFrame`. This bypasses heavy canvas geometry redraw loops, making resizing instantaneous.
   - Progress bars use a static width (`width=110`) to avoid canvas recreation operations during drags.
   - The sidebar column has weight 0 and `grid_propagate(False)`, locking its size and keeping redraw calls isolated strictly to the main content container.

2. **Native Windows Startup Toggle**:
   - The settings tab contains a **"Run at Windows Startup"** toggle switch.
   - Natively reads/writes HKEY_CURRENT_USER registry keys using the `winreg` library to register the app in the official Windows Startup list. No administrator privileges are required.

3. **Database Folder Selection & Migration**:
   - Defaults to saving your logs in `Documents\LocalTimeTracker\tracker.db`.
   - You can choose a custom directory from the settings tab. The app will automatically stop the tracker, copy the database file to the new folder to preserve history, write the path to `AppData\LocalTimeTracker\config.json`, update pointers, and resume tracking.

4. **Application Exclusions ("Untracked")**:
   - Flag any executable as `"Untracked"` (🚫) in settings.
   - The background tracker thread keeps a dynamic cache of these exclusions and ignores active window monitoring when they are in focus.
   - Excluded apps are filtered out from screen time totals, daily averages, and the app breakdown list.

5. **Visual Grid & History Calendars**:
   - Active screen times are grouped by category (Productivity, Entertainment, Distraction, and Uncategorized) in a **2x2 grid of cards** on the dashboard.
   - Clicking the **Weekly Average** card opens a daily log breakdown of the last 7 days.
   - Clicking the **Monthly Screen Time** card opens an interactive calendar showing screen times for each day of the month, color-coded by activity (similar to GitHub squares), with month-navigation controls (`<` and `>`).
...
class SystemTrayApp:
    ...
    def start(self):
        ...
        self.icon = pystray.Icon("TimeTrackerIcon", create_tray_icon_image(), "Time Tracker", menu=menu)
